...
int testResOverlap(){
    ...
    //testing different days, duration does not overlap, should not overlap
    Date d1(2022,9,30);
    Date d2(2022,9,28);
    Reservation resA("Joe",d1,3);
    Reservation resB("Bob",d2, 2);
    if(!resOverlap(resA,resB,false)) score =0;

    //Testing different months, should not overlap- edge case: last day of d1 is the first day of d2 and Frebruary
    Date d3(2022,2,27);
    Date d4(2022, 3, 2);
    Reservation resC("Barry",d3,3);
    Reservation resD("Allen", d4, 5);
    if(!resOverlap(resC,resD,false)) score =0;

    //Testing different months, should not overlap - edge case: February leap year- however current date file does not account for leap
    Date d5(2020,2,27);
    Date d6(2020,3,1);
    Reservation res5("Steve",d5,3);
    Reservation res6("Rogers", d6,4);
    if(!resOverlap(res5,res6,true)) score=0;

    //Testing different months, should overlap - edge case: February leap year
    Date d7(2020,2,27);
    Date d8(2020, 3,2);
    Reservation res7("Tony", d7, 4);
    Reservation res8("Stark", d8, 3);
    if (!resOverlap(res8,res7, true)) score =0;

    //Testing long reservations, different years, should overlap
    Date d9(2021,1,1);
    Date d10(2022,1,1);
    Reservation res9("Bucky", d9, 366);
    Reservation res10("Barnes", d10, 10);
    if (!resOverlap(res9, res10, true)) score =0;
